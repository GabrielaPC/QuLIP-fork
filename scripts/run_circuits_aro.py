...
train_img = load_pkl(os.path.join(img_path, 'aro_imgenc_train__512.pkl'))
...
params = load_pkl(os.path.join(root_path, 'aro_runs/06_19_17;11;51/model.lt'))
all_params_dict = {}
img_params_dict = {}
...
for i in range(data_size):
    qc_pos_txt, output_qubits, params_dict = tn2qiskit(test_einsum[i][0], test_einsum[i][1], meas_output=False, all_params_dict=all_params_dict)

    for i in range(qc_pos_txt.num_qubits):
        if i not in output_qubits:
          qc_pos_txt.measure(i, i)  

    qc_neg_txt, output_qubits, params_dict = tn2qiskit(test_einsum[i][0], test_einsum[i][1], meas_output=False, all_params_dict=all_params_dict)

    for i in range(qc_neg_txt.num_qubits):
        if i not in output_qubits:
          qc_neg_txt.measure(i, i)  
    ...
    # qc_pos.draw("mpl")
    pos_f = run_circuit(qc_pos, params_dict, qc_pos_txt.num_qubits)

    qc_neg = qc_neg_txt.copy()
    ...
    qc_neg.compose(qc_img, qreg_img, inplace=True)
    qc_neg.h(qreg_anc)
    for i in range(len(output_qubits)):
    ...
    # qc_pos.draw("mpl")
    neg_f = run_circuit(qc_neg, params_dict, qc_neg_txt.num_qubits)
