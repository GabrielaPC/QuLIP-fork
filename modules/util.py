...
def fs_distance(state1, state2, precision=-1):
    inner_product = torch.sum(state1 * state2.conj(), dim=1)
    
    if precision >=0:
        return torch.round((torch.full(inner_product.size(), torch.pi/2) - torch.acos(inner_product.abs().clamp(0,1))), decimals=precision)
